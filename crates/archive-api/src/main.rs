...
use crate::replay::{Resolver, Rewriter, WarcReader};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let intelligence_engine: Arc<dyn archive_intelligence::IntelligenceEngine> =
        Arc::new(archive_intelligence::HybridEngine::new(llm_engine));

    let notification_dispatcher: Arc<dyn archive_notification::NotificationDispatcher> =
        Arc::new(archive_notification::MultiChannelDispatcher::new());
        ...
        Ok(rows) => {
            ...
            let snapshots = rows
                ...
                .map(|row| {
                    ...
                    let intensity = if digest != last_digest && !last_digest.is_empty() {
                        1.0
                    } else {
                        0.1
                    };
...
async fn get_diff(
    ...
    let s1 = state
        .resolver
        .resolve(&params.url, ts_from)
        .await
        .ok()
        .flatten();
    let s2 = state
        .resolver
        .resolve(&params.url, ts_to)
        .await
        .ok()
        .flatten();
    ...
    let d1 = state
        .warc_reader
        .read_record(&s1.warc_file, s1.offset, s1.length)
        .await
        .unwrap_or_default();
    let d2 = state
        .warc_reader
        .read_record(&s2.warc_file, s2.offset, s2.length)
        .await
        .unwrap_or_default();
...
async fn get_frontier_health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let rows = sqlx::query(
        r#"
...
async fn resolve_v1(
    ...
    let ts = match ReplayUrl::parse(&params.at, &params.url) {
        ...
        Err(_) => {
            return Response::builder()
                .status(400)
                .body("Invalid timestamp format".into())
                .unwrap()
        }
    };
    ...
    match state.resolver.resolve(&params.url, ts).await {
        ...
        Ok(None) => (
            ...
            "No snapshot found for given URL and time",
        )
            .into_response(),
        Err(e) => {
